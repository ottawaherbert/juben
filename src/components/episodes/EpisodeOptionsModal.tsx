...
interface EpisodeOptionsModalProps {
  ...
  onSelectOption: (episodes: any[]) => void;
  onClose: () => void;
}

export const EpisodeOptionsModal: React.FC<EpisodeOptionsModalProps> = ({
  generatedOptions,
  onSelectOption,
  onClose
}) => {
          ...
          >
            <div className="flex items-center gap-4 mb-6 shrink-0">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-400">AI 为您提供了以下不同走向的方案，请选择最符合您意图的一个。</p>
              </div>
            </div>
              ...
              {generatedOptions.map((option, index) => (
                <div key={index} className="bg-neutral-950 border border-neutral-800 rounded-2xl p-5 hover:border-emerald-500/50 transition-colors">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button
                      onClick={() => onSelectOption(option.episodes)}
                  ...
                  <div className="space-y-3">
                    {option.episodes.slice(0, 3).map((ep: any, epIdx: number) => (
                      <div key={epIdx} className="bg-neutral-900 rounded-xl p-3">
                        <div className="text-sm font-bold text-white mb-1">{ep.title}</div>
                        <div className="text-xs text-neutral-400 line-clamp-2">{ep.inspiration}</div>
                      </div>
                    ))}
                    {option.episodes.length > 3 && (
                      <div className="text-xs text-neutral-500 text-center pt-2">
                        ... 以及其他 {option.episodes.length - 3} 个部分
                      </div>
                    )}
                  </div>
                </div>
