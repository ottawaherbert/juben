...
export default function BeatBoard() {
  ...
  // Ensure an active episode is selected in fast mode (usually the first one)
  useEffect(() => {
    if (currentProject && currentProject.episodes.length > 0 && !activeEpisode) {
      setActiveEpisodeId(currentProject.episodes[0].id);
    }
  }, [currentProject, activeEpisode, setActiveEpisodeId]);
  ...
  if (!activeEpisode) {
    if (currentProject && currentProject.episodes.length === 0) {
      return (
        <div className="flex-1 flex flex-col items-center justify-center text-neutral-500 space-y-4">
          ...
          <button
            onClick={() => {
              ...
              useProjectStore.getState().addEpisode(newEpisode);
            }}
